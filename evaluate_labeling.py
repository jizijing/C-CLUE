...
def load_result_1(golden_filename):
    result_dict = {}
    with open(golden_filename) as gf:
        for line in gf:
            ...
            print(spo_list)
            spo_result = []
            o = del_bookname(spo_list['object'].lower())
            s = del_bookname(spo_list['subject'].lower())
            spo_result.append((s, spo_list['predicate'], o))
            spo_result = set(spo_result)
            result_dict[sent] = spo_result
    return result_dict

def load_result_2(predict_filename):
    result_dict = {}
        ...
        for line in gf:
            ...
            if (spo_list!=''):
                ...
                for item in spo_list:
                    o = del_bookname(item.get('object').lower())
                    s = del_bookname(item.get('subject').lower())
                    spo_result.append((s, item.get('predicate'), o))
            spo_result = set(spo_result)
            result_dict[sent] = spo_result
    return result_dict
...
def calc_pr(predict_filename, golden_filename):
    ...
    ret_info = {}
    # load test dataset
    golden_dict= load_result_1(golden_filename)
    # load predict result
    predict_result = load_result_2(predict_filename)
    # evaluation
...
if __name__ == '__main__':
    ...
    golden_filename = os.path.join(
        os.path.abspath(os.path.join(os.path.dirname(__file__), "../../raw_data_guwen")), "test_data.json")
