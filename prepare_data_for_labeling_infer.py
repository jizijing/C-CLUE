...
def replace_empty_infer_predicate_to_three_possible_values(predicate_score_value):
    label_list = ['任职', '隶属于', '子', '同名于', '号', '作战', '位于', '依附', '名', '讨伐', '去往', '升迁', '杀', '管理','弟','出生地', '葬于', '属于', '父', '归属', '朋友', '兄', '作', '姓', '字']
...
def prepare_data_for_subject_object_labeling_infer(predicate_classifiction_input_file_dir,
    ...
    for text, token_in, token_in_not_UNK, predicate_predict, predicate_score_value in zip(text_file, token_in_file, token_in_not_UNK_file, predicate_predict_file, predicate_score_value_file):
        count_line += 1
...
if __name__=="__main__":
    predicate_classifiction_input_file_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "../predicate_classifiction/new_classification_data_guwen/test"))
    ...
    out_file = os.path.abspath(os.path.join(os.path.dirname(__file__), "../subject_object_labeling/sequence_labeling_data_guwen/test"))
